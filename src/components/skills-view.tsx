...
function SkillCard({ skill, onClick }: { skill: Skill; onClick: () => void }) {
  ...
  return (
    <button type="button" onClick={onClick} className={cn("w-full rounded-xl border p-3.5 text-left transition-all hover:scale-[1.01]", skill.eligible ? "border-foreground/[0.06] bg-foreground/[0.02] hover:border-foreground/[0.12]" : "border-foreground/[0.04] bg-foreground/[0.01] opacity-70 hover:opacity-100 hover:border-foreground/[0.08]")}>
      <div className="flex items-start gap-3">
        <span className="text-xl leading-none mt-0.5">{skill.emoji || "\u26A1"}</span>
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <p className="text-[13px] font-semibold text-foreground/90">{skill.name}</p>
            {skill.eligible ? <CheckCircle className="h-3 w-3 shrink-0 text-emerald-500" /> : <XCircle className="h-3 w-3 shrink-0 text-muted-foreground/60" />}
            {skill.disabled && <span className="rounded bg-muted px-1.5 py-0.5 text-[8px] text-muted-foreground">DISABLED</span>}
