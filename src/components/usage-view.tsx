...
function TokenBar({ input, output, total, maxTotal }: {
  ...
  return (
    <div className="h-3 overflow-hidden rounded-full bg-foreground/[0.04]">
  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between text-[10px]">
        <span className="text-muted-foreground/60">Context Window</span>
  ...
  return (
    <div className="rounded-xl border border-foreground/[0.06] bg-card p-4">
      <div className="flex items-center gap-2.5">
        <div className={cn("flex h-9 w-9 items-center justify-center rounded-lg", accent || "bg-foreground/[0.04]")}>{icon}</div>
...
function ModelCard({ m, maxTokens, isPrimary, isFallback, rank }: {
  ...
  return (
    <div className={cn("rounded-xl border transition-all", isPrimary ? "border-violet-500/20 bg-violet-500/[0.03]" : "border-foreground/[0.06] bg-card/90")}>
      <button type="button" onClick={() => setExpanded(!expanded)} className="flex w-full items-center gap-3 p-4 text-left">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <p className="truncate text-sm font-semibold text-foreground/90">{shortModel(m.model)}</p>
          ...
          <p className="text-[10px] text-muted-foreground/60">{m.sessions} sessions &bull; {m.agents.length} agent{m.agents.length !== 1 ? "s" : ""} &bull; Last used {fmtAgo(m.lastUsed)}</p>
        </div>
        <div className="shrink-0 text-right">
          <p className="text-sm font-bold text-foreground/90">{fmtTokens(m.totalTokens)}</p>
          <p className="text-[10px] text-muted-foreground/60">{shareOfTotal}% of total</p>
        </div>
        ...
        <div className="border-t border-foreground/[0.04] p-4 space-y-4">
          <div className="grid grid-cols-3 gap-4">
            <div className="rounded-lg bg-foreground/[0.03] p-3 text-center">
              ...
              <p className="text-[9px] text-muted-foreground/40">{fmtTokensLong(m.inputTokens)}</p>
            </div>
            <div className="rounded-lg bg-foreground/[0.03] p-3 text-center">
              <div className="flex items-center justify-center gap-1"><ArrowUp className="h-3 w-3 text-violet-400" /><p className="text-[10px] text-muted-foreground">Output</p></div>
              <p className="mt-1 text-base font-bold text-violet-400">{fmtTokens(m.outputTokens)}</p>
              <p className="text-[9px] text-muted-foreground/40">{fmtTokensLong(m.outputTokens)}</p>
            </div>
            <div className="rounded-lg bg-foreground/[0.03] p-3 text-center">
              <div className="flex items-center justify-center gap-1"><Gauge className="h-3 w-3 text-emerald-400" /><p className="text-[10px] text-muted-foreground">Avg Context</p></div>
              <p className="mt-1 text-base font-bold text-emerald-400">{m.avgPercentUsed}%</p>
              <p className="text-[9px] text-muted-foreground/40">of {fmtTokens(m.contextTokens)} window</p>
            </div>
          </div>
          <div>
            <p className="mb-1 text-[10px] text-muted-foreground/60">Input / Output ratio</p>
          ...
          <div>
            <p className="mb-1.5 text-[10px] font-medium uppercase tracking-wider text-muted-foreground/60">Used by Agents</p>
            <div className="flex flex-wrap gap-1.5">
              {m.agents.map((a) => <span key={a} className="rounded-md bg-foreground/[0.04] px-2 py-1 text-[11px] font-medium text-foreground/70">{a}</span>)}
            </div>
          </div>
        </div>
      )}
...
function SessionRow({ s }: { s: SessionEntry }) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg border border-foreground/[0.04] bg-card/70 px-3 py-2.5">
      <span className="shrink-0 rounded bg-foreground/[0.05] px-1.5 py-0.5 text-[9px] font-semibold text-muted-foreground">{s.agentId}</span>
      <div className="min-w-0 flex-1">
        ...
        <p className="text-[9px] text-muted-foreground/60">{shortModel(s.model)} &bull; {fmtAgo(s.updatedAt)}{s.thinkingLevel ? " \u00b7 think:" + s.thinkingLevel : ""}</p>
      </div>
      <div className="shrink-0 text-right">
        <p className="text-[11px] font-semibold text-foreground/70">{fmtTokens(s.totalTokens)}</p>
...
export function UsageView() {
  ...
  if (loading) return <div className="flex flex-1 items-center justify-center"><RefreshCw className="h-5 w-5 animate-spin text-muted-foreground/60" /></div>;

  if (error || !data) return (
    <div className="flex flex-1 flex-col items-center justify-center gap-3 text-muted-foreground">
      <AlertCircle className="h-8 w-8 text-red-400" />
      <p className="text-sm">Failed to load usage data</p>
      <button type="button" onClick={fetchData} className="rounded-lg bg-foreground/5 px-3 py-1.5 text-xs text-foreground/70 hover:bg-foreground/10">Retry</button>
    </div>
  );
    ...
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between border-b border-foreground/[0.06] px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-foreground">Usage & Analytics</h1>
            <p className="text-xs text-muted-foreground/60">Token consumption, model performance, session analytics</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex rounded-lg border border-foreground/[0.06] bg-card">
            {(Object.keys(PERIOD_LABELS) as Period[]).map((p) => (
              <button key={p} type="button" onClick={() => setPeriod(p)} className={cn("px-3 py-1.5 text-[11px] font-medium transition-colors first:rounded-l-lg last:rounded-r-lg", period === p ? "bg-violet-500/15 text-violet-400" : "text-muted-foreground hover:text-foreground/70")}>{PERIOD_LABELS[p]}</button>
            ))}
          </div>
          <button type="button" onClick={() => { setLoading(true); fetchData(); }} className="rounded-lg border border-foreground/[0.06] bg-card p-2 text-muted-foreground hover:bg-muted hover:text-foreground/70"><RefreshCw className="h-3.5 w-3.5" /></button>
        </div>
      </div>
        ...
        <div className="mx-auto max-w-7xl space-y-6 p-6">
          ...
          <div className="rounded-xl border border-foreground/[0.06] bg-card/90 p-5">
            ...
            <div className="grid grid-cols-4 gap-4">
              {(["last1h", "last24h", "last7d", "allTime"] as Period[]).map((p) => {
                ...
                return (
                  <button key={p} type="button" onClick={() => setPeriod(p)} className={cn("rounded-xl border p-4 text-left transition-all", isActive ? "border-violet-500/30 bg-violet-500/[0.05]" : "border-foreground/[0.04] bg-foreground/[0.02] hover:border-foreground/[0.08]")}>
                    <p className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground/60">{PERIOD_FULL[p]}</p>
                    <p className="mt-1 text-xl font-bold text-foreground">{fmtTokens(b.total)}</p>
                    <div className="mt-2"><TokenBar input={b.input} output={b.output} total={b.total} maxTotal={data.buckets.allTime.total} /></div>
                    <p className="mt-2 text-[10px] text-muted-foreground/60">{b.sessions} session{b.sessions !== 1 ? "s" : ""}</p>
                  </button>
                );
              })}
            </div>
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            <div className="lg:col-span-2 space-y-3">
              ...
              {modelBreakdown.length === 0 ? <p className="text-sm text-muted-foreground/60">No model usage data yet</p> :
            ...
            <div className="space-y-4">
              {modelConfig && (
                <div className="rounded-xl border border-foreground/[0.06] bg-card/90 p-4">
                  <h3 className="mb-3 flex items-center gap-1.5 text-xs font-semibold text-foreground/70"><Target className="h-3.5 w-3.5 text-violet-400" />Model Stack</h3>
                  <div className="space-y-2">
                    ...
                    {modelConfig.fallbacks.map((f, i) => (
                      <div key={f} className="flex items-center gap-2 rounded-lg bg-foreground/[0.03] px-3 py-2">
                        <span className="rounded-full bg-foreground/[0.08] px-1.5 py-0.5 text-[8px] font-bold text-muted-foreground">{i + 2}</span>
                        <code className="text-[11px] text-muted-foreground">{shortModel(f)}</code>
                        <span className="ml-auto text-[9px] text-muted-foreground/60">fallback</span>
                      </div>
                    ))}
                    {modelConfig.imageModel && (
                      <div className="mt-1 flex items-center gap-2 rounded-lg border border-dashed border-foreground/[0.06] px-3 py-2">
                        <span className="text-[9px] text-muted-foreground/60">Image:</span>
                        <code className="text-[11px] text-muted-foreground">{shortModel(modelConfig.imageModel)}</code>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {modelConfig && Object.keys(modelConfig.aliases).length > 0 && (
                <div className="rounded-xl border border-foreground/[0.06] bg-card/90 p-4">
                  <h3 className="mb-3 flex items-center gap-1.5 text-xs font-semibold text-foreground/70"><Hash className="h-3.5 w-3.5 text-amber-400" />Aliases</h3>
                  <div className="space-y-1.5">
                    {Object.entries(modelConfig.aliases).map(([alias, model]) => (
                      <div key={alias} className="flex items-center gap-2 text-[11px]">
                        ...
                        <code className="truncate text-muted-foreground">{shortModel(model)}</code>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {modelConfig && modelConfig.authProviders.length > 0 && (
                <div className="rounded-xl border border-foreground/[0.06] bg-card/90 p-4">
                  <h3 className="mb-3 flex items-center gap-1.5 text-xs font-semibold text-foreground/70"><Key className="h-3.5 w-3.5 text-emerald-400" />Auth Providers</h3>
                  <div className="space-y-2">
                    {modelConfig.authProviders.map((ap) => (
                      <div key={ap.provider} className="flex items-center gap-2 rounded-lg bg-foreground/[0.03] px-3 py-2">
                        <Shield className="h-3 w-3 text-emerald-500" />
                        <span className="text-[11px] font-medium text-foreground/70">{ap.provider}</span>
                        <span className="ml-auto rounded bg-foreground/[0.05] px-1.5 py-0.5 text-[9px] text-muted-foreground">{ap.authKind}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {agentModels.length > 0 && (
                <div className="rounded-xl border border-foreground/[0.06] bg-card/90 p-4">
                  <h3 className="mb-3 flex items-center gap-1.5 text-xs font-semibold text-foreground/70"><Users className="h-3.5 w-3.5 text-cyan-400" />Agent Model Config</h3>
                  <div className="space-y-3">
                    {agentModels.map((am) => (
                      <div key={am.agentId}>
                        <p className="mb-1 text-[11px] font-semibold text-foreground/70">{am.agentId}</p>
                        <div className="rounded-lg bg-foreground/[0.03] px-3 py-2 space-y-1">
                          <div className="flex items-center gap-1.5 text-[10px]">
                            <span className="text-violet-400">Primary:</span>
                            <code className="text-muted-foreground">{shortModel(am.primary)}</code>
                          </div>
                          {am.fallbacks.length > 0 && (
                            <div className="flex flex-wrap gap-1">
                              {am.fallbacks.map((f, i) => <span key={f} className="rounded bg-foreground/[0.05] px-1.5 py-0.5 text-[9px] text-muted-foreground">{i + 1}. {shortModel(f)}</span>)}
                            </div>
                          )}
                        </div>
                      </div>
              ...
              {sessionFileSizes.length > 0 && (
                <div className="rounded-xl border border-foreground/[0.06] bg-card/90 p-4">
                  <h3 className="mb-3 flex items-center gap-1.5 text-xs font-semibold text-foreground/70"><HardDrive className="h-3.5 w-3.5 text-pink-400" />Session Storage</h3>
                  <div className="space-y-2">
                    {sessionFileSizes.map((sf) => (
                      <div key={sf.agentId} className="flex items-center justify-between rounded-lg bg-foreground/[0.03] px-3 py-2 text-[11px]">
                        <span className="font-medium text-foreground/70">{sf.agentId}</span>
                        <div className="text-right">
                          <span className="font-mono text-muted-foreground">{fmtBytes(sf.sizeBytes)}</span>
                          <span className="ml-2 text-muted-foreground/60">{sf.fileCount} file{sf.fileCount !== 1 ? "s" : ""}</span>
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <div className="rounded-xl border border-foreground/[0.06] bg-card/90 p-5">
              <h2 className="mb-4 flex items-center gap-2 text-sm font-semibold text-foreground/90"><Users className="h-4 w-4 text-cyan-400" />Usage by Agent</h2>
              {agentBreakdown.length === 0 ? <p className="text-sm text-muted-foreground/60">No agent data</p> : (
                <div className="space-y-3">
                  {agentBreakdown.map((a) => (
                    <div key={a.agentId}>
                      <div className="flex items-center justify-between">
                        <div><p className="text-[12px] font-semibold text-foreground/90">{a.agentId}</p><p className="text-[10px] text-muted-foreground/60">{a.sessions} sessions &bull; {a.models.join(", ")}</p></div>
                        <p className="text-[12px] font-bold text-foreground/70">{fmtTokens(a.totalTokens)}</p>
                      </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-xl border border-foreground/[0.06] bg-card/90 p-4 text-center">
                  ...
                  <p className="text-[10px] text-muted-foreground/60">Avg tokens/session</p>
                </div>
                <div className="rounded-xl border border-foreground/[0.06] bg-card/90 p-4 text-center">
                  <Gauge className="mx-auto h-5 w-5 text-violet-400" />
                  <p className="mt-2 text-lg font-bold text-foreground">{totals.inputTokens + totals.outputTokens > 0 ? tokenRatio(totals.inputTokens, totals.outputTokens).inPct + ":" + tokenRatio(totals.inputTokens, totals.outputTokens).outPct : "\u2014"}</p>
                  <p className="text-[10px] text-muted-foreground/60">Global I/O ratio</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Session Feed */}
          <div className="rounded-xl border border-foreground/[0.06] bg-card/90 p-5">
            <h2 className="mb-4 flex items-center gap-2 text-sm font-semibold text-foreground/90"><Clock className="h-4 w-4 text-blue-400" />Recent Sessions<span className="ml-auto text-[10px] font-normal text-muted-foreground/60">Top {Math.min(sessions.length, 50)} by recency</span></h2>
            <div className="space-y-1.5">
